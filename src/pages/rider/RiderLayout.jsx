...
import { Bike, LogOut, PackageCheck, History } from 'lucide-react'
import { useAuth } from '../../context/AuthContext'
import { CAFE } from '../../lib/format'
import HeaderLeft from '../../components/HeaderLeft'

const tabs = [
  { to: '/rider', end: true, icon: PackageCheck, label: 'Deliveries' },
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-2xl flex-col">
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-cafe-line bg-cafe-bg/95 px-4 py-3 backdrop-blur">
        <HeaderLeft homePath="/rider" tabs={tabs} brand={
          <div className="flex items-center gap-2">
            <Bike className="text-cafe-accent" size={22} />
            <div>
              <p className="text-xs text-cafe-muted">Rider · {CAFE.name}</p>
              <h1 className="text-lg font-black leading-none text-cafe-accent">{profile?.full_name || 'Rider'}</h1>
            </div>
          </div>
        } />
